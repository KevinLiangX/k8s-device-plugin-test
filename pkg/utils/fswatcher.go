...
// Keep watching kubelet restart device plugin when kubelet restarted
func WatchKubelet(stop chan<- struct{}) error {
	...
	if err != nil {
		return errors.WithMessagef(err, "Unable to add path %s to watch", pluginapi.KubeletSocket)
	}
	return nil
}
